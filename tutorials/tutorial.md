...
`./run-backend.sh` 在未覆盖时默认 **`LOCAL_AUTH_ENABLED=true`**、**`LOCAL_AUTH_ALLOW_REGISTRATION=true`**（见根目录 `run-backend.sh` 与 `backend/.env.example`）。**不包含公网开放注册**；仅面向本机/内网部署。

**本地账号操作步骤：**

1. 重启后端后打开 **`http://127.0.0.1:5173/register`**，创建首个用户（自动 **admin**）或后续用户（默认 **operator**）。  
2. **`/login`** 使用用户名密码登录；会话为 HttpOnly Cookie **`perilla_session`**。  
3. 侧栏底部显示当前用户名/角色；本地账号可点 **退出登录**。  
4. 可选 **`AUTH_REQUIRE_LOGIN=true`**：未登录时 API 返回 401，前端跳转登录页。

**OIDC 操作步骤（摘要）：** 配置 `OIDC_*` 后，在 **企业与合规** 页发起登录，回调 **`/auth/callback`**；前端将 access token 存入 `localStorage` 并以 Bearer 调用网关。详见该页说明与 `backend/api/auth_oidc.py`。
...
以下 **Settings** 子节仍适用于界面偏好与 API Key（与登录方式正交）。
...
编辑页工具栏 **「发布」**（非灰色「部署」占位）：先 **保存为新版本**，再点发布 → 弹出 **发布门禁**（服务端预检 + 契约 diff）。版本列表页每条版本也可 **发布**。若按钮仍为灰色「部署」，请硬刷新前端；创建页需 **先保存** 再发布。

#### D. 与审计头字段的关系（可选了解）

网关请求还会自动携带 **`X-User-Id`**：本地账号登录后为该用户的 UUID；未启用本地登录时缺省为 `default`（亦可由 `localStorage` 键 **`ai_platform_user_id`** 覆盖）。API Key 身份见 **§8.2 B**。
...
### 8.6 一键导入 / 导出 Bundle（平台包 · 工作流包 · ZIP）

本节对应侧栏 **「一键导入」**（路由 **`/import`**）与网关 **`/api/v1/import/*`**。用于在内网 PoC、环境迁移或团队间共享 **Agent + 知识库 + Skill + MCP 模板 + 工作流** 等组合资产。

#### A. 三种入口（与演示包的关系）

| 场景 | UI | API / 文件 |
|------|-----|------------|
| 内置演示 / 工作流包 | **导入 → 内置目录** | `GET /api/v1/import/catalog` → `POST /api/v1/import/run` |
| 上传或粘贴自定义包 | **导入 → 上传文件 / 粘贴 JSON** | `POST /api/v1/import/upload`（`.json` 或 **`.zip`**）或 `POST /api/v1/import/run-body` |
| 从当前环境导出 | **导出** 页签 | `POST /api/v1/import/export/discover` → `POST /api/v1/import/export` |

画布 **「演示项目（多智能体）」** 仍只在 **工作流 → 新建** 中导入轻量 DAG，不替代平台完整包。

#### B. 权限与前置条件

1. **写操作**：导入 / 导出要求 **operator+**（`require_platform_write`），与演示包导入一致。  
2. **模型**：平台包导入时 `__AUTO_LLM__` / `__AUTO_EMBEDDING__` 会选用已注册的首个 LLM / Embedding；导出时可选择 **占位符**（`use_model_placeholders: true`，默认）以便跨环境导入。  
3. **MCP 管理 API**（Probe / 单独改 Server）仍可能需要 **管理员 API Key**（见 **§8.5 A**）；bundle 内 MCP 走持久化写入，默认 **`enabled: false`** 模板。

#### C. 导入（JSON 与 ZIP）

1. **JSON**：根对象须 `schema_version: 1`；`bundle_type` 为 `platform` 或 `workflow`，或可自动推断。  
2. **ZIP 结构**（推荐，尤其含知识库二进制文档时）：

   ```text
   my-pack.zip
   ├── my-pack.platform-bundle.json   # 或 *.bundle.json（工作流包）
   └── documents/
       └── *.md / *.pdf / …
   ```

3. 平台包文档：单文件 JSON 上传时请用内联 **`content`**；**`content_path`** 仅在 ZIP 内相对于包根目录解析。  
4. 预检：`POST /api/v1/import/validate`（粘贴 JSON 前可点 UI「校验格式」）。  
5. 引用：包内用 **`$import:<bundle_key>`** 关联 KB / Skill / Agent / 工作流（见 `demos/platform/BUNDLE_SCHEMA.md`）。

**curl 示例（内置平台包）：**

```bash
curl -sS -X POST "http://127.0.0.1:8000/api/v1/import/run" \
  -H "Content-Type: application/json" \
  -H "X-Tenant-Id: default" \
  -H "X-User-Id: <your-user-uuid>" \
  -d '{"kind":"platform","bundle_id":"rag-research-verify","publish_workflows":false,"wait_document_index":true}'
```

**curl 上传 ZIP：**

```bash
curl -sS -X POST "http://127.0.0.1:8000/api/v1/import/upload" \
  -H "X-Tenant-Id: default" \
  -H "X-User-Id: <your-user-uuid>" \
  -F "file=@./my-export.zip"
```

（浏览器内操作由前端 `apiFetch` 自动处理 **CSRF**；脚本调用写接口需按 **§13** 携带 Cookie 与 `X-CSRF-Token`。）

#### D. 导出当前环境

1. 在 **`/import` → 导出** 选择 **主工作流**（须已有 **latest 版本** 且含 DAG）。  
2. 点击 **扫描依赖**：根据 DAG 中 **Agent 节点** 与 Agent 上的 **Skill / 知识库** 汇总 ID。  
3. 填写 **`bundle_id`、名称**，选择 **ZIP**（含 `documents/`）或 **JSON**。  
4. 下载后可在另一租户 / 另一台机器用 **上传 ZIP** 或 **`/import/run-body`** 还原。

**curl 发现依赖：**

```bash
curl -sS -X POST "http://127.0.0.1:8000/api/v1/import/export/discover" \
  -H "Content-Type: application/json" \
  -H "X-Tenant-Id: default" \
  -d '{"workflow_ids":["<workflow-uuid>"]}'
```
...
curl -sS -X POST "http://127.0.0.1:8000/api/v1/import/export?download=true" \
  ...
  -d '{
    ...
    "bundle_id":"my-export",
    "name":"我的环境快照",
    "workflow_ids":["<workflow-uuid>"],
    "use_model_placeholders": true,
...
启动网关后打开 **`http://127.0.0.1:8000/docs`**，标签 **import** 下可见：

- `GET /api/v1/import/catalog`  
- `POST /api/v1/import/validate`  
- `POST /api/v1/import/run` · `/run-body` · `/upload`  
- `POST /api/v1/import/export/discover` · `/export`  

#### F. 常见问题

- **导出后 Agent 节点仍是旧 UUID**：请导出 **平台包**（`kind: platform`），不要仅导出工作流包；工作流包 DAG 保留运行时 ID，仅适合同环境备份 DAG。  
- **PDF 文档导入失败**：使用 **ZIP** 导出并保留 `documents/`；或转为 `.md` / `.txt` 内联 `content`。  
- **MCP 工具未自动出现**：导出含 `mcp_servers` + `mcp_tool_imports` 时，Server 默认关闭；到 **Settings → MCP** 启用后 **Probe → Import**（与 **§8.5** 一致）。
...
除上文 **§9.2～9.3** 的 API 创建/执行外，控制台编辑器已支持完整节点库，建议按 UI 路径上手：

1. 打开 **Workflow 新建/编辑** 页（`/workflow/.../edit` 或创建入口）。  
2. 在工具栏下方选择 **「高价值编排模板（OmX）」**（如 **澄清·计划·验收**、**并行小队 Fork/Join**、**Ralph 验证环**）→ **导入编排模板**。  
3. 为 **LLM** 节点配置 **`model_id`** 或 **`model_tier`**（`low` / `standard` / `thorough`）；为 **Embedding** 选择嵌入模型。  
4. **预检** → **保存** → **发布版本**（生产建议）→ **Run**，输入 JSON 入参（如 `{ "task": "..." }`）。  
5. 在运行页查看 **Timeline**；若含 **approval** 节点可能 **PAUSED**，需走审批 API（见下文 HITL）。

**节点能力速查**（配置 JSON 与误区见专文，不在此重复）：

| 类型 | 用途 |
|------|------|
| Fork / Join | 扇出并行分支 / 汇聚（Join 默认等待全部分支） |
| Parallel 限流 | `max_parallel`，**不是** Fork |
| Checkpoint | 验收门禁（`required_keys` 等） |
| Verify Loop | Ralph 式「执行→验收→重试直至通过」 |
| HTTP / Python | 经 `http.request`、`python.run` 工具 |
| Variable | 写入 `workflow_variables` |

**权威文档**（与实现对齐，避免三处漂移）：

- 概览与 OmX 对照：`docs/workflow/WORKFLOW_ORCHESTRATION_OVERVIEW_ZH.md`  
- 节点配置模板：`docs/workflow/WORKFLOW_NODE_CONFIG_GUIDE.md`  
- 本地 Case D/E：`docs/workflow/WORKFLOW_TEST_CASES_LOCAL.md`  
- 索引入口：`tutorials/tutorial-index.md` **§1.2**
