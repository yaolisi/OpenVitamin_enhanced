...
* **工具失败反思（suggest_only）**：在 `AgentDefinition.model_params.tool_failure_reflection` 中设置 `{"enabled": true, "mode": "suggest_only"}` 时，V2 `PlanBasedExecutor` 在 **原子 SKILL 步骤失败** 后调用一次当前 Agent 的 `model_id` 生成诊断建议，**不**自动改参、不重跑工具；结果写入同一会话 Trace：`event_type` 为 `reflection_suggestion`，并合并到该步 `outputs["reflection_suggestion"]`。同一次 plan 执行内反思调用次数有上限（`MAX_REFLECTIONS_PER_PLAN_RUN`，见 `tool_failure_suggest.py`），防止连续失败拖垮推理网关。成功写入的 `output_data` 会带 `reflection_index`（本执行内第几次）与 `max_reflections_per_run`。REST 创建/更新 Agent 时 `api/agents` 会校验 `tool_failure_reflection` 仅含 `enabled` / `mode` 且 `mode` 仅能为 `suggest_only`。与根目录 **User-in-Control** 一致，默认关闭，需显式开启。实现见 `core/agent_runtime/reflection/tool_failure_suggest.py`。
